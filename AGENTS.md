...
- Pre-release: `v{upstream}+{N}~pre` (unstable channel)
...
```
v0.2.0+1~pre < v0.2.0+1 < v0.2.0+2~pre < v0.2.0+2 < v0.3.0+1~pre < v0.3.0+1
```
...
5. CI creates pre-release tag `v{upstream}+{N}~pre` with .deb
6. CI creates draft release tag `v{upstream}+{N}` (no assets initially)
...
✅ Debian-compliant versioning (tilde sorts correctly)
