...
function App() {
  ...
  const renderView = () => {
    switch (route.view) {
      ...
      case "installed":
        return (
          <InstalledView onNavigateToPackage={handleNavigateToPackage} onRemove={handleRemove} />
        );

      case "updates":
        return <UpdatesView onNavigateToPackage={handleNavigateToPackage} />;
  ...
  return (
    <Page id="apt" className="pf-m-no-sidebar">
      ...
      <PageSection hasBodyWrapper={false}>{renderView()}</PageSection>
    </Page>
  );
}
